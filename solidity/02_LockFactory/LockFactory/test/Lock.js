...
describe("Lock", function () {
  async function deployOneYearLockFixture() {
    ...
    const lock = await Lock.deploy(owner.address, unlockTime, { value: lockedAmount });
  ...
  describe("Deployment", function () {
    ...
    it("Should fail if the unlockTime is not in the future", async function () {
      ...
      const Lock = await ethers.getContractFactory("Lock");
      await expect(
          Lock.deploy("0x0000000000000000000000000000000000000001", latestTime, { value: 1 })
  ...
  describe("Withdrawals", function () {
    ...
    describe("Transfers", function () {
      it("Should transfer the funds to the owner", async function () {
        ...
        await expect(() =>
            lock.withdraw()
        ).to.changeEtherBalances([owner, await lock.getAddress()], [lockedAmount, -lockedAmount]);
      });
    });
  });
});
